    ...
    def get_inspection_summary(inspection):
        ...
        return {
            ...
            'completion_percentage': (
                sum(1 for f in inspection.findings.all()
...
class FindingService:
    ...
    def get_findings_by_equipment(equipment):
        ...
        return Finding.objects.filter(
            inspection__equipment=equipment
        ).order_by('-created_at')

    @staticmethod
    def get_unresolved_findings():
        """Obtiene hallazgos sin acciones correctivas asignadas"""
        return Finding.objects.exclude(
            corrective_actions__status='completed'
        ).distinct()
...
class DashboardService:
    ...
    def get_equipment_stats():
        ...
        return Equipment.objects.annotate(
            ...
            critical_inspections=Count('inspections', filter=Q(inspections__criticality='critical')),
        )
