...
def dashboard(request):
    ...
    recent_inspections = Inspection.objects.select_related("equipment", "inspector").order_by(
        "-inspection_date"
    )[:10]
    ...
    inspections_by_week = (
        Inspection.objects.annotate(week=TruncWeek("inspection_date"))
        .values("week")
...
class InspectionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Inspection.objects.select_related("equipment", "inspector").order_by(
            "-inspection_date"
        )
...
class EquipmentListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(Q(folio__icontains=search) | Q(name__icontains=search))
...
class FindingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "inspections:inspection-detail", kwargs={"pk": self.object.inspection.pk}
        )
...
class CorrectiveActionCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "inspections:inspection-detail", kwargs={"pk": self.object.finding.inspection.pk}
        )
