...
from apps.accounts.permissions import Rol, RolRequiredMixin

from . import alegra, wompi
from .models import DatosFacturacion, Pago, PlanServicio, Suscripcion, calcular_n_meses
...
class PagoPortalView(RolRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["datos_facturacion"] = (
            suscripcion.datos_facturacion
            if suscripcion and suscripcion.datos_facturacion_id
            else None
        )
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if event == "transaction.updated":
                ...
                logger.info(
                    f"WOMPI webhook: tx={tx_id} status={status} created={created} ya_estaba_aprobado={ya_estaba_aprobado}"
                )
