...
class UniqueConstraintPagoTests(TestCase):
    ...
    def test_wompi_transaction_id_duplicado_no_vacio_falla(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            Pago.objects.create(
                suscripcion=self.suscripcion, monto=100000, wompi_transaction_id="TX-1"
            )
...
class WebhookIdempotenciaTests(TestCase):
    ...
    @staticmethod
    def _payload(
        tx_id="tx-webhook-1", status="APPROVED", amount_cents=10000000, reference="REF-WEBHOOK-1"
    ):
        return {
